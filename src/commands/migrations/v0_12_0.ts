...
function phaseCBackfillLinks(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    execSync('gbrain extract links --source db' + childGlobalFlags(), { stdio: 'inherit', timeout: 1_800_000, env: process.env });
...
function phaseDBackfillTimeline(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    execSync('gbrain extract timeline --source db' + childGlobalFlags(), { stdio: 'inherit', timeout: 1_800_000, env: process.env });
